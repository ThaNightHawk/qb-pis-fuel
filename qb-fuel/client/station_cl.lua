if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    local function SpawnPickupVehicles()
        ...
        if truckToSpawn then
            ...
            SetVehicleFuelLevel(spawnedDeliveryTruck, 100.0)
    ...
    RegisterNetEvent('qb-fuel:stations:client:purchasereserves', function(data)
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                local reserves = lib.inputDialog('Køb reserve', {
					{ type = "input", label = 'Nuværrende pris',
					default = Config.FuelReservesPrice .. ',- Per Liter',
					disabled = true },
					{ type = "input", label = 'Nuværrende reserve',
					default = Currentreserveamount,
					disabled = true },
					{ type = "input", label = 'Nødvendig reserve',
					default = Config.MaxFuelReserves - Currentreserveamount,
					disabled = true },
					{ type = "slider", label = 'Pris for fuld opfyldning: ' ..math.ceil(GlobalTax((Config.MaxFuelReserves - Currentreserveamount) * Config.FuelReservesPrice) + ((Config.MaxFuelReserves - Currentreserveamount) * Config.FuelReservesPrice)).. ',-',
					default = Config.MaxFuelReserves - Currentreserveamount,
					min = 0,
                ...
                local reserves = exports['qb-input']:ShowInput({
                    header = Lang:t("input_purchase_reserves_header_1") .. Lang:t("input_purchase_reserves_header_2") .. Currentreserveamount.."/"..Config.MaxFuelReserves.. Lang:t("input_purchase_reserves_header_3") ..
    ...
    RegisterNetEvent('qb-fuel:stations:client:changefuelprice', function(data)
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                local fuelprice = lib.inputDialog('Brændstofpriser', {
                    { type = "input", label = 'Nuværrende pris',
                    default = Comma_Value(StationFuelPrice) .. ',- Per Liter',
                    disabled = true },
                    { type = "number", label = 'Indtast ny pris per liter',
    ...
    RegisterNetEvent('qb-fuel:stations:client:changestationname', function() -- Menu for changing the label of the owned station.
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                local NewName = lib.inputDialog('Skift navn', {
                    { type = "input", label = 'Nuværrende navn',
                    default = Config.GasStations[CurrentLocation].label,
                    disabled = true },
                    { type = "input", label = 'Indtast nyt navn',
            ...
            if Config.Ox.Menu then
                    ...
                    options = {
                        {
                            ...
                            description = 'Køb din reserve brændstof her!',
                            ...
                            metadata = {
                                {label = 'Reserve tank: ', value = ReserveLevels..Lang:t("menu_manage_reserves_footer_1")..Config.MaxFuelReserves},
                            },
                            disabled = ReservesNotBuyable,
                        },
                        {
                            title = Lang:t("menu_alter_fuel_price_header"),
                            description = "Jeg vil skifte prisen på brændstoffen!",
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            ...
                            metadata = {
                                {label = 'Nuværrende pris: ', value = Comma_Value(StationFuelPrice)..",-"..Lang:t("input_alter_fuel_price_header_2")},
                            },
                ...
                exports['qb-menu']:openMenu({
                    ...
                    },
                    {
                        ...
                        icon = "fas fa-usd",
                        txt = "Jeg vil skifte prisen på brændstof! <br> Lige nu koster det "..StationFuelPrice..",-"..Lang:t("input_alter_fuel_price_header_2") ,
                        params = {
                            event = "qb-fuel:stations:client:changefuelprice",
                            args = {
                                location = location,
    ...
    RegisterNetEvent('qb-fuel:stations:client:WithdrawFunds', function(data)
        ...
        if CanOpen then
            ...
            Wait(50)
            if Config.Ox.Input then
                local Withdraw = lib.inputDialog('Hæv kapitel', {
                    { type = "input", label = 'Nuværrende kapital',
                    default = Comma_Value(StationBalance)..",-",
                    disabled = true },
                    { type = "number", label = 'Hæv beløb',
                    },
                })
    ...
    RegisterNetEvent('qb-fuel:stations:client:DepositFunds', function(data)
        ...
        if CanOpen then
            ...
            Wait(50)
            if Config.Ox.Input then
                local Deposit = lib.inputDialog('Indsæt kapital', {
                    { type = "input", label = 'Nuværrende kapital',
                    default = Comma_Value(StationBalance)..",-",
                    disabled = true },
                    { type = "number", label = 'Indsæt beløb',
                    },
                })
                ...
                options = {
                    {
                        ...
                        description = 'Jeg vil godt købe denne tankstation!',
                        icon = "fas fa-usd",
                        arrow = true, -- puts arrow to the right
                        ...
                        metadata = {
                            {label = 'Tankstationen koster ', value = Comma_Value(costofstation)..",- "..Lang:t("menu_purchase_station_header_2")},
                        },
                    },
                    {
                        title = Lang:t("menu_header_close"),
