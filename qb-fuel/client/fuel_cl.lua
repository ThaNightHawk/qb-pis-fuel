...
if Config.ShowNearestGasStationOnly then
    ...
    CreateThread(function()
        ...
        while true do
            ...
            local location = 0
            local label = "Tankstation" -- Prevent nil just in case, set default name.
    ...
    CreateThread(function()
        ...
        for i = 1, #Config.GasStations, 1 do
            ...
            -- Check if the blip for this location already exists
            if not GasStationBlips[location] then
    ...
    if Config.Ox.Input then
        if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
            if ReserveLevels < maxfuel then
                ...
                fuel = lib.inputDialog('Tankstation', {
                    {type = "input", label = 'Brændstof pris', default = FuelPrice .. ',- per liter', disabled = true},
                    {type = "input", label = 'Nuværrende tankbeholdning', default = finalfuel .. ' liter', disabled = true},
                    {type = "input", label = 'Nødvendigt for fuld tank', default = maxfuel .. ' liter', disabled = true},
                    {type = "input", label = 'Brændstof tilgængelig på tankstationen', default = ReserveLevels, disabled = true},
                    {type = "slider", label = 'Pris for fuld tank: ' .. wholetankcostwithtax .. ',-', default = ReserveLevels, min = 0, max = ReserveLevels},
                })
                ...
                fuel = lib.inputDialog('Tankstation', {
                    {type = "input", label = 'Brændstof pris', default = FuelPrice .. ',- per liter', disabled = true},
                    {type = "input", label = 'Nuværrende tankbeholdning', default = finalfuel .. ' liter', disabled = true},
                    {type = "input", label = 'Required For A Full Tank', default = maxfuel .. ' liter', disabled = true},
                    {type = "slider", label = 'Pris for fuld tank: ' .. wholetankcostwithtax .. ',-', default = maxfuel, min = 0, max = maxfuel},
                })
            ...
            fuel = lib.inputDialog('Tankstation', {
                {type = "input", label = 'Brændstof pris', default = FuelPrice .. ',- per liter', disabled = true},
                {type = "input", label = 'Nuværrende tankbeholdning', default = finalfuel .. ' Per Liter', disabled = true},
                {type = "input", label = 'Nødvendigt for fuld tank', default = maxfuel .. ' liter', disabled = true},
                {type = "slider", label = 'Pris for fuld tank: ' .. wholetankcostwithtax .. ',-', default = maxfuel, min = 0, max = maxfuel},
            })
        ...
        if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
            if ReserveLevels < maxfuel then
                ...
                fuel = exports['qb-input']:ShowInput({
                    header = "Vælg mængden af brændstof<br>Nuværrende pris: " ..
                    FuelPrice .. ",- / Liter <br> Nuværrende beholdning: " .. finalfuel .. " Liter <br> Fuld tank koster: " ..
                    ...
                    inputs = {{
                        ...
                        isRequired = true,
                        name = 'amount',
                        text = 'Der er ' .. ReserveLevels .. ' liter til rådighed.'
                    }}
                })
            else
                fuel = exports['qb-input']:ShowInput({
                    ...
                    inputs = {{
                        ...
                        isRequired = true,
                        name = 'amount',
                        text = 'Tank kan rumme ' .. maxfuel .. ' liter mere.'
                    }}
                })
            end
        else
...
RegisterNetEvent('qb-fuel:client:SendMenuToServer', function(type)
    ...
    if NotElectric then
        ...
        if type == 'special' then
            header = "Genopfyldning af brændstof"
            RefuelingType = 'special'
        else
        ...
        if HasSyphon then
            ...
            if reason == "syphon" then
                if Config.Ox.Input then
                    ...
                    local syphon = exports['qb-input']:ShowInput({
                        header = "Vælg hvor meget du vil stjæle.",
                        submitText = "Begyn tapning",
                        inputs = {
                            {
                                type = 'number',
                                isRequired = true,
                                name = 'amount',
                                text = 'Du kan stjæle ' .. Stealstring .. 'L fra køretøjet.'
                            }
                        }
                    })
                    if syphon then
            ...
            elseif reason == "refuel" then
                ...
                if Config.Ox.Input then
                    ...
                    if refuel then
                        if tonumber(refuel.amount) == 0 then QBCore.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuel.amount) < 0 then QBCore.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuel.amount) > 100 then QBCore.Functions.Notify("Du kan kun tanke 100L!", 'error') return end
